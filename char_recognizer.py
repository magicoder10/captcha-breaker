...
model = Sequential()
model.add(
    Conv2D(100, (5, 5), activation='relu', batch_input_shape=(None, image_height, image_width, 3),
           data_format="channels_last", name='conv_1'))
model.add(MaxPooling2D(pool_size=(2, 2), data_format='channels_last', name='max_pool_1'))
model.add(Conv2D(50, (3, 3), activation='relu', data_format='channels_last', name='max_pool_2'))
...
history_callback = model.fit(X_train, Y_train, validation_data=(X_test, Y_test), batch_size=32, epochs=30, verbose=1,
                             initial_epoch=0)

model_json = model.to_json()
with open('models/%d.model.json' % len(X), 'w') as model_json_file:
    model_json_file.write(model_json)
model.save_weights('models/%d.weights.hdf5')
